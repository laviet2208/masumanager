...
class delete_product extends StatefulWidget {
  ...
  const delete_product({super.key, required this.product});
...
class _delete_productState extends State<delete_product> {
  ...
  Future<void> delete_product() async {
    ...
    await databaseRef.child('Product').child(widget.product.id).remove();
      ...
      actions: <Widget>[
        !loading ? TextButton(
          onPressed: () async {
            ...
            await deleteImage('Product/' + widget.product.id + '.png');
