...
class recharge_money_for_shipper extends StatefulWidget {
  final shipperAccount account;
  const recharge_money_for_shipper({Key? key, required this.account}) : super(key: key);

  @override
  State<recharge_money_for_shipper> createState() => _recharge_money_for_shipperState();
}

class _recharge_money_for_shipperState extends State<recharge_money_for_shipper> {
  ...
  Future<void> change_admin_money() async {
    try {
      DatabaseReference databaseRef = FirebaseDatabase.instance.reference();
      await databaseRef.child('Area/' + widget.account.area).child('money').set(area.money);
      setState(() {
        loading = false;
      });
