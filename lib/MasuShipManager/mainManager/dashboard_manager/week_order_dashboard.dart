...
class week_order_dashboard extends StatefulWidget {
  const week_order_dashboard({super.key});

  @override
  State<week_order_dashboard> createState() => _week_order_dashboardState();
}

class _week_order_dashboardState extends State<week_order_dashboard> {
    ...
    reference.child("Order").once().then((DatabaseEvent event) {
      ...
      orders.forEach((key, value) {
        if (value['resCost'] != null) {
          ...
          if (DateTime(order.timeList[1].year, order.timeList[1].month, order.timeList[1].day).weekday == DateTime.now().weekday) {
            food_order_list.add(order);
          }
        } else if (value['receiver'] != null) {
          expressOrder order = expressOrder.fromJson(value);
          if (DateTime(order.S2time.year, order.S2time.month, order.S2time.day).weekday == DateTime.now().weekday) {
