...
class add_free_restaurant extends StatefulWidget {
  const add_free_restaurant({super.key});

  @override
  State<add_free_restaurant> createState() => _add_free_restaurantState();
}

class _add_free_restaurantState extends State<add_free_restaurant> {
  bool loading = false;
  List<String> type_list = ['5 sao','Ăn vặt','Bún phở','Cơm','Khuyến mãi','Món nhậu','Nước uống','Thức ăn nhanh','Trà sữa'];
  String chosenStatus = '';
  ShopAccount account = ShopAccount(id: '', createTime: getCurrentTime(), lockStatus: 0, name: '', phone: '', type: 0, password: '', closeTime: getCurrentTime(), openTime: getCurrentTime(), openStatus: 0, discount_type: 2, area: '', location: Location(placeId: '', description: '', longitude: 0, latitude: 0, mainText: '', secondaryText: ''), listDirectory: [], money: 0);
  ...
  Future<void> uploadImageToFirebaseStorage(Uint8List imageBytes, String imageName) async {
    try {
      ...
      FirebaseStorage.instance.ref().child('Restaurant/$imageName.png');
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('Thêm nhà hàng tự do'),
      content: Container(
